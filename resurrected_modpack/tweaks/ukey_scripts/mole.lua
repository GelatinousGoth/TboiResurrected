...
function mod:IsTimeUp()
	local limit = mod.TimeLimit

	-- More time for XL floors
	if Game():GetLevel():GetCurses() & LevelCurse.CURSE_OF_LABYRINTH > 0 then
		limit = mod.TimeLimitXL
	end
	return Game().TimeCounter > limit
end
...
function mod:CustomSlotCallbacks()
	local slots = Isaac.FindByType(EntityType.ENTITY_SLOT, mod.MoleVariant)
...
function mod:TrySpawnMole()
	...
	if Isaac.GetChallenge() == Challenge.CHALLENGE_NULL -- Not in a challenge
	and not Game():IsGreedMode() -- Not in Greed Mode
	and level:GetAbsoluteStage() == LevelStage.STAGE1_1 and level:GetStageType() < StageType.STAGETYPE_REPENTANCE -- On the first floor
	and room:GetType() == RoomType.ROOM_SHOP and room:IsFirstVisit() -- First entering a shop
	and not mod:IsTimeUp() -- Under the time limit
	and not mod:DoesAnyoneHaveUnlockKey() and not mod.SpawnedKey then -- The key hasn't been spawned before
		local pos = room:GetGridPosition(26)
