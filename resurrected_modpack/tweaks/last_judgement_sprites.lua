...
LastJudgementSprites = TR_Manager:RegisterMod("Last Judgement Sprites", 1, true)
...
for _, s in ipairs(mod.Scripts) do
    include("resurrected_modpack.tweaks.last_judgement_sprites."..s)
end
...
mod:AddCallback(ModCallbacks.MC_PRE_NPC_RENDER, function(_, npc)
    if npc.Variant == 0 then
        local sprite = npc:GetSprite()
        for i = 1, 2 do
            if sprite:GetLayer(i):GetColor().A > 0 then
...
mod:AddCallback(ModCallbacks.MC_POST_PROJECTILE_UPDATE, function(_, proj)
    if proj.Velocity:Length() > 0.05 then
        proj.SpriteRotation = proj.Velocity:GetAngleDegrees() - 90
    end
