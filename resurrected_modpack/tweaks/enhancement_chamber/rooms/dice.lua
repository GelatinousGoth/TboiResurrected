...
function mod:dicePostLevel()
    self.Data.diceTriggered = false
    self.Data.diceRestart = false
end
...
function mod:diceEffectUpdate(effect)
    ...
    if not effect:GetData().ec_dice_check then
        -- Dice ID
        ...
        if not self.Data.diceTriggered then
            ...
            if distance < 75 then
                ...
                if MinimapAPI then self:roomIconUpdate() end -- Minimap icon
                -- Vanilla dice sprite
                if effect.SubType < 100 then
                    if diceID == 5 then self.Data.diceRestart = true end -- 5-pip check
