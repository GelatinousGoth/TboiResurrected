...
mod.CurrentModName = "Voided Items"
...
function VoidedItems:AddOnetimeUse(item)
    mod.onetime_use[item] = true
end
--Example: VoidedItems:AddOnetimeUse(CollectibleType.COLLECTIBLE_CONTRABAND)

mod:AddOnetimeUse(CollectibleType.COLLECTIBLE_FORGET_ME_NOW)
mod:AddOnetimeUse(CollectibleType.COLLECTIBLE_BLUE_BOX)
mod:AddOnetimeUse(CollectibleType.COLLECTIBLE_DIPLOPIA)
mod:AddOnetimeUse(CollectibleType.COLLECTIBLE_PLAN_C)
mod:AddOnetimeUse(CollectibleType.COLLECTIBLE_MAMA_MEGA)
mod:AddOnetimeUse(CollectibleType.COLLECTIBLE_EDENS_SOUL)
mod:AddOnetimeUse(CollectibleType.COLLECTIBLE_MYSTERY_GIFT)
mod:AddOnetimeUse(CollectibleType.COLLECTIBLE_SACRIFICIAL_ALTAR)
mod:AddOnetimeUse(CollectibleType.COLLECTIBLE_DAMOCLES)
mod:AddOnetimeUse(CollectibleType.COLLECTIBLE_ALABASTER_BOX)
mod:AddOnetimeUse(CollectibleType.COLLECTIBLE_GENESIS)
mod:AddOnetimeUse(CollectibleType.COLLECTIBLE_DEATH_CERTIFICATE)
mod:AddOnetimeUse(CollectibleType.COLLECTIBLE_R_KEY)

mod:AddCallback(ModCallbacks.MC_POST_GAME_STARTED, function(_)
if ARACHNAMOD then
    mod:AddOnetimeUse(Isaac.GetItemIdByName("Best Bud Ball"))
    mod:AddOnetimeUse(Isaac.GetItemIdByName("Testament"))
end
if EclipsedMod then
    mod:AddOnetimeUse(EclipsedMod.enums.Items.FloppyDisk)
    mod:AddOnetimeUse(EclipsedMod.enums.Items.VHSCassette)
end
if Epiphany then
    mod.PlayerTypesWithOffset[Epiphany.PlayerType.ISAAC] = true
    mod:AddOnetimeUse(Epiphany.Item.FIRST_PAGE.ID)
    mod:AddOnetimeUse(Epiphany.Item.SURPRISE_BOX.ID)
end
if FiendFolio then
    mod:AddOnetimeUse(CollectibleType.COLLECTIBLE_CONTRABAND)
    mod:AddOnetimeUse(FiendFolio.ITEM.COLLECTIBLE.WRONG_WARP)
    mod:AddOnetimeUse(FiendFolio.ITEM.COLLECTIBLE.RAT_POISON)
    mod:AddOnetimeUse(FiendFolio.ITEM.COLLECTIBLE.ERRORS_CRAZY_SLOTS)
end
if LibraryExpanded then
    mod:AddOnetimeUse(LibraryExpanded.Item.CERTIFICATE)
    mod:AddOnetimeUse(LibraryExpanded.Item.BLANK_BOOK.ID0)
    mod:AddOnetimeUse(LibraryExpanded.Item.BLANK_BOOK.ID1)
    mod:AddOnetimeUse(LibraryExpanded.Item.BLANK_BOOK.ID2)
    mod:AddOnetimeUse(LibraryExpanded.Item.BLANK_BOOK.ID3)
    mod:AddOnetimeUse(LibraryExpanded.Item.BLANK_BOOK.ID4)
end
if Retribution then
    mod:AddOnetimeUse(Retribution.ITEMS.SCULPTING_CLAY)
    mod:AddOnetimeUse(Retribution.ITEMS.PUMPKIN_MASK)
    mod:AddOnetimeUse(Retribution.ITEMS.SHATTERED_DICE)
    mod:AddOnetimeUse(Retribution.ITEMS.BOTTLED_FAIRY)
    mod:AddOnetimeUse(Retribution.ITEMS.NEVERLASTING_PILL)
end
if RepentancePlusMod then
    mod:AddOnetimeUse(RepentancePlusMod.CustomCollectibles.BIRTH_CERTIFICATE)
end
if Sheriff then mod:AddOnetimeUse(Sheriff.Items.MOAB.ID) end
if SacredDreams then
    mod.PlayerTypesWithOffset[SDMod.PlayerType.PLAYER_GUARD] = true
end
end)
...
-- Item tracking
function mod:TryAddToVoidedItems(player, item)
...
local abyss = false
local TryToVoid = {}
mod:AddCallback(ModCallbacks.MC_USE_ITEM, function(_, item, _, player, useflags)
    if useflags & UseFlag.USE_VOID > 0 then
        if mod.onetime_use[item] then
            local d = player:GetData()
...
mod:AddCallback(ModCallbacks.MC_PRE_USE_ITEM, function(_, _, _, player)
    if player.QueuedItem.Item then
        mod:TryAddToVoidedItems(player, player.QueuedItem.Item.ID)
    end
...
mod:AddCallback(ModCallbacks.MC_USE_ITEM, function(_, _, _, player)
    if abyss then abyss = false
        ...
        for _, pickup in pairs(TryToVoid) do
            ...
            if pickup:Exists() then mod:TryAddToVoidedItems(player, pickup.SubType) end
        end
        TryToVoid = {}
    end
end, void)
...
mod:AddCallback(ModCallbacks.MC_POST_GAME_STARTED, function(_, hasSave)
    if not hasSave then
        mod.itemSprites = {}
    elseif mod:HasData() then
        local data = json.decode(Isaac.LoadModData(mod))
        for i=0, 4 do
...
local IndexedPlayers = {
	[1] = {PlayerPtr = nil,
		ScreenPos = function(player, slot)
			...
			if mod.PlayerTypesWithOffset[player:GetPlayerType()] then
...
local function TryGenerateSprite(item)
    if not mod.itemSprites[item] then
        local itemConfigItem = itemConfig:GetCollectible(item)
        ...
        mod.itemSprites[item] = itemSprite
    end
end
        ...
        for i = 1, #IndexedPlayers do
            ...
            if player:HasCollectible(void) then
                ...
                for item, _ in pairs(items) do
                    ...
                    local sprite = mod.itemSprites[item]
...
if REPENTOGON then
    mod:AddCallback(ModCallbacks.MC_POST_RENDER, function(_) mod:UpdatePlayers() end)
    mod:AddCallback(ModCallbacks.MC_HUD_RENDER, function(_) if Game():GetHUD():IsVisible() then mod:RenderItems() end end)
else include("rendering") end
