...
mod.CurrentModName = "Shop Parrot"

local monster = {}
monster.name = "Shop Parrot"
monster.type = mod.registry.entities.keepah.type
monster.variant = mod.registry.entities.keepah.variant
...
local is_not_shop = function()
	return mod.room:GetType() ~= RoomType.ROOM_SHOP
end
...
monster.npc_update = function(self, ent, data, sprite)
			...
			if data.run_from:IsDead() or not data.run_from:IsVisible() then
				...
				if sprite:IsEventTriggered("Flap") then
					...
					if string.match(sprite:GetAnimation(),"Sweat") and is_not_shop() then 
						...
						creep.CollisionDamage = mod.level:GetAbsoluteStage() / 4.0 + 5.0
	...
	if data.talk_sprite ~= nil and mod.save_manager.get_config("MuteShopBird","false") == "false" then 
		...
		if volume_mod > 0 then 
			...
			if data.run_from ~= nil then 
				if data.time % 7 == 6 then -- panic
					ent:PlaySound(mod.registry.sounds.keepah_panic, 0.45*volume_mod, 0, false, 0.9-0.125+pitch_off*0.25)
				end
			else 
				if (data.donate or 0) > 0 then 
					if data.time % 5 == 3 then -- donate
						ent:PlaySound(mod.registry.sounds.keepah, 0.55*volume_mod, 0, false, 1.1-0.125+pitch_off*0.25)
					end
				elseif (data.buy or 0) > 0 then -- buy
					if data.time % 6 == 5 then 
						ent:PlaySound(mod.registry.sounds.keepah, 0.55*volume_mod, 0, false, 1-0.125+pitch_off*0.25)
					end
				elseif data.time % 7 == 4 then -- conversation
					ent:PlaySound(mod.registry.sounds.keepah_panic, 0.45*volume_mod, 0, false, 1.0-0.0625+pitch_off*0.125)
				end	
			end	
		end
	end
...
monster.npc_post_render = function(self, ent, offset)
	local data = mod.get_ent_data(ent)
...
local config_parrot = function(parrot, appear2)
	...
	if appear2 then
		...
		if mod.room:GetType() == RoomType.ROOM_SHOP then 
			parrot:GetSprite():Play("Appear2",true)
		else 
			parrot:GetSprite():Play("Idle",true)
		end
	else
		local data = mod.get_ent_data(parrot)
		data.bubble = Sprite()
		data.bubble:Load("gfx/famil_parrot.anm2", true)
		data.talk_sprite = "BubbleAppear"
		data.bubble:Play(data.talk_sprite,true)
...
monster.new_room = function(self)
	if true then --mod.room:GetType() == RoomType.ROOM_SHOP then
		if mod.save_manager.get_config("ShopParrot","true") == "true" then 
			local kc_count = mod.util.total_item_count(mod.registry.trinkets.keepah_card, true)
			local count = (mod.room:GetType() == RoomType.ROOM_SHOP and 1 --shop count
							or mod.keepah_mode == true and mod.level:GetAbsoluteStage() or 0) --april fools count
							+ kc_count * 2 --keepah card count

			for i=1,count do 
				local parrot = Isaac.Spawn(monster.type,monster.variant,0,mod.room:FindFreePickupSpawnPosition(mod.room:GetCenterPos()),Vector.Zero,nil)
				config_parrot(parrot, not mod.room:IsFirstVisit() or is_not_shop())	

				if not is_not_shop() and i > 1 then 
					mod.get_ent_data(parrot).replace_sprite = true
				end
			end
		end

		if mod.save_manager.get_config("ShopFog","true") == "true" and not is_not_shop() then 
			local poses = {
				{pos=mod.room:GetCenterPos(),vel=RandomVector()*0.05},
				{pos=mod.room:GetTopLeftPos(),vel=Vector(math.abs(RandomVector().X),math.abs(RandomVector().Y)*0.25)*0.05+Vector(0.05,0)},
				{pos=mod.room:GetBottomRightPos(),vel=Vector(math.abs(RandomVector().X),math.abs(RandomVector().Y)*0.25)*-0.05-Vector(0.05,0)}
			}
...
monster.player_collide = function(self, player,ent,entfirst) 
	...
	if not data.parrot_talk or data.parrot_talk - player.FrameCount <= 0 then 
		if ent.Type == EntityType.ENTITY_PICKUP then-- and ent.Variant == PickupVariant.PICKUP_SHOPITEM then -- buy
			mod.util.macro_on_enemies(nil,monster.type,monster.variant,-1,function(parrot)
				mod.get_ent_data(parrot).buy = 1
			end)
		...
		elseif ent.Type == EntityType.ENTITY_SLOT and ent.Variant == (SlotVariant or donation_variant).DONATION_MACHINE then -- donate
			mod.util.macro_on_enemies(nil,monster.type,monster.variant,-1,function(parrot)
				mod.get_ent_data(parrot).donate = 1
			end)	
