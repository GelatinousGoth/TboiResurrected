...
mod.CurrentModName = "Unique Progress Bar Icon"
---@class ModReference
...
---@return IsaacIcon
---@overload fun(playerType: PlayerType)
function mod.CreateIcon(player)
	local playerType = type(player) == "number" and player or player:GetPlayerType()
	local iconData = {
...
function mod:LoadIsaacIcons()
	...
	for _, player in ipairs(PlayerManager.GetPlayers()) do
		...
		local iconData = mod.CreateIcon(player)
		Isaac.RunCallback(mod.Callbacks.POST_CREATE_ICON, iconData, player)
...
function mod:RenderIsaacIcons(renderPos, isStageAPI)
	...
	Isaac.RunCallback(mod.Callbacks.POST_ICONS_RENDER, isaacIcons, shadowLocations, renderPos)
end
...
function mod:OnNightmareRender()
	...
	if currentNightmareFrame == 0 then
		Isaac.RunCallback(mod.Callbacks.PRE_ICONS_INIT)
		NightmareScene.GetProgressBarSprite():GetLayer(1):SetVisible(false)
		mod:LoadIsaacIcons()
		shadowLocations:SetFrame(tostring(#isaacIcons), 0)
		Isaac.RunCallback(mod.Callbacks.POST_ICONS_INIT, isaacIcons, shadowLocations)
	end
